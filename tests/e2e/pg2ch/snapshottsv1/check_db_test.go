...
func testSnapshot(t *testing.T, source *provider_postgres.PgSource, target clickhouse_model.ChDestination) {
	...
	require.NoError(t, storagecomparison.CompareStorages(t, source, target, storagecomparison.NewCompareStorageParams().WithEqualDataTypes(pg2ch.PG2CHDataTypesComparatorV1)))
}
