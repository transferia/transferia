...
func TestSnapshotAndIncrement(t *testing.T) {
	...
	require.NoError(t, storage.WaitEqualRowsCount(t, databaseName, "rsv_null_in_json", storagecomparison.GetSampleableStorageByModel(t, Source), storagecomparison.GetSampleableStorageByModel(t, Target), 60*time.Second))
	require.NoError(t, storagecomparison.CompareStorages(t, Source, Target, storagecomparison.NewCompareStorageParams().WithEqualDataTypes(pg2ch.PG2CHDataTypesComparator).WithPriorityComparators(pg2ch.ValueJSONNullComparator)))
}
