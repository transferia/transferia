...
func SnapshotAndIncrement(t *testing.T) {
	...
	makeIncrementActions(t, "__test2")
...
func Increment(t *testing.T) {
	...
	_, err = srcConn.Exec(ctx, fmt.Sprintf("INSERT INTO public.%s (id, name) VALUES (3000, 'test3test3') on conflict do nothing ;", "__test1"))
	require.NoError(t, err)
	_, err = srcConn.Exec(ctx, fmt.Sprintf("UPDATE public.%s SET id = 1002 WHERE name = 'test1test1';", "__test1"))
	require.NoError(t, err)
