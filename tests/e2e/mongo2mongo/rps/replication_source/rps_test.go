...
type RpsTestParameters struct {
	SrcParamGen func() *provider_mongo.MongoSource
}
...
func RpsTest(t *testing.T) {
	...
	for testName, testParam := range map[string]RpsTestParameters{
		"PerDatabase": {SrcParamGen: func() *provider_mongo.MongoSource {
			src := *Source
			src.ReplicationSource = provider_mongo.MongoReplicationSourcePerDatabase
			return &src
		}},
		"PerDatabaseFullDocument": {SrcParamGen: func() *provider_mongo.MongoSource {
			src := *Source
			src.ReplicationSource = provider_mongo.MongoReplicationSourcePerDatabaseFullDocument
			return &src
		}},
		"PerDatabaseUpdateDocument": {SrcParamGen: func() *provider_mongo.MongoSource {
			src := *Source
			src.ReplicationSource = provider_mongo.MongoReplicationSourcePerDatabaseUpdateDocument
			return &src
		}},
		"Oplog": {SrcParamGen: func() *provider_mongo.MongoSource {
			src := *Source
