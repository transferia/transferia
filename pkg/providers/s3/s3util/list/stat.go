...
type stat struct {
	...
	skippedBcsMineButOutOfRWindow int64
	notSkipped                    int64
}
...
func (s *stat) log(logger log.Logger) {
	logger.Infof(
		"ListNewMyFiles finished, time_elapsed:%s, skippedBcsNotMatched:%d, skippedBcsNotMine:%d, skippedBcsMineButKnown:%d, skippedBcsMineButOutOfRWindow:%d, notSkipped:%d",
		s.duration(),
		s.skippedBcsNotMatched,
		s.skippedBcsNotMine,
		s.skippedBcsMineButKnown,
		s.skippedBcsMineButOutOfRWindow,
		s.notSkipped,
	)
}
...
func newStat() *stat {
	return &stat{
		...
		skippedBcsMineButOutOfRWindow: int64(0),
		notSkipped:                    int64(0),
	}
}
