...
func objectsToArrFileEntry(in []object) []file.File {
	result := make([]file.File, 0, len(in))
	for _, obj := range in {
		result = append(result, file.File{
			FileName:       obj.Name,
			FileSize:       0,
			LastModifiedNS: obj.LastModified.UTC().UnixNano(),
		})
	}
	return result
}
...
func (s *ObjectFetcherSQS) FetchObjects(inReader reader.Reader) ([]file.File, error) {
	...
	for {
		...
		if len(obj) == 0 && len(s.toDelete) == 0 {
			...
			return objectsToArrFileEntry(objectList), nil
		}
...
func (s *ObjectFetcherSQS) Close() error {
	return nil
}
