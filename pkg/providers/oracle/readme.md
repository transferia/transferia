...
Для параллельной загрузки в рамках одного воркера используйте `ProcessCount` в `UploadShardParams` рантайма — воркер будет одновременно обрабатывать до `ProcessCount` ORA_HASH-шардов из FCFS-пула.
