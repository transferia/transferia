...
### dt.synthetic.types.policy

У колонок, добавленных трансформерами, может отсутствовать специфичный для источника `OriginalType`. По умолчанию debezium serializer отклоняет такие колонки. Значение `common` разрешает сериализовать колонки с пустым `OriginalType` общим механизмом на основе их YT-типа.

Настройка не влияет на типы с префиксами `pg:`, `mysql:` и `ydb:`. Непустой неизвестный `OriginalType` всегда приводит к ошибке. Ошибки сериализации неподдерживаемых общим механизмом YT-типов также не обрабатываются через `dt.unknown.types.policy`.

Возможные значения: "fail", "common"
Значение по умолчанию: "fail"
