...
func (p *DebeziumImpl) DoOne(partition abstract.Partition, buf []byte, offset uint64, writeTime time.Time) ([]byte, abstract.ChangeItem) {
	if len(buf) == 0 {
		return nil, generic_parser.NewUnparsed(partition, strings.ReplaceAll(partition.Topic, "/", "_"), []byte{}, "debezium parser received empty message", 0, offset, writeTime)
	}
	if p.isUsedSchemaRegistry && buf[0] != 0 {
		return nil, generic_parser.NewUnparsed(partition, strings.ReplaceAll(partition.Topic, "/", "_"), buf, fmt.Sprintf("debezium parser configured with SR, but magic byte is not NULL, byte:%d", buf[0]), 0, offset, writeTime)
	}
