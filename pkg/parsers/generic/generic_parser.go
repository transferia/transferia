...
const (
	ColNameTimestamp = unparsed.ColNameTimestamp
	ColNamePartition = unparsed.ColNamePartition
	ColNameOffset    = unparsed.ColNameOffset
	ColNameIdx       = unparsed.ColNameIdx
)
...
var (
	UnparsedCols   = unparsed.UnparsedCols
	UnparsedSchema = unparsed.UnparsedSchema
)

func IsGenericUnparsedSchema(schema *abstract.TableSchema) bool {
	return unparsed.IsGenericUnparsedSchema(schema)
}
...
func tableName(part abstract.Partition, name string) string {
	return unparsed.TableName(part, name)
}
...
func NewUnparsed(partition abstract.Partition, name string, line []byte, reason string, idx int, offset uint64, writeTime time.Time) abstract.ChangeItem {
	return unparsed.NewUnparsed(partition, name, line, reason, idx, offset, writeTime)
}
